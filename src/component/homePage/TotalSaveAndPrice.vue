<template>
  <div class="saveandprice">
    <router-link :to="'/SecondPage'">
      <div class="Continues">ادامه خرید از {{countVendor}} غرفه</div>
    </router-link>
      <div class="twoproducts">
      <div>
     
      <h4 class="totalprice">{{ itemsOfVendor }}</h4>
       <h4 class="currency-unit">
          <span class="currency">توما</span>
          <span class="currency2">ن</span>
        </h4>
      </div>
      <span> جمع مبلغ برای {{ items }} کالا </span>
    </div>
  </div>
</template>
...
<script>
export default {
  props: ["countVendor", "itemsOfVendor","items"],
};
</script>
<style scoped>
.currency {
